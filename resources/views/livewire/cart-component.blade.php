    ...
    <div class=" main-content-area">
            ...
            <ul class="products-cart">
                ...
                <li class="pr-cart-item">
                    ...
                    <div class="product-name">
                        <a class="link-to-product" href="{{route('product.details',['slug'=>$item->model->image])}}">{{$item->model->name}}</a>
                    </div>
                    @foreach($item->options as $key => $value)
                        <div styles="verticlal-align:middle;width:180px;">
                            <p><b>{{$key}}:{{$value}}</b></p>
                        </div>
                    @endforeach
        ...
        <div class="summary">
            <div class="order-summary">
                ...
                <p class="summary-info"><span class="title">Subtotal</span><b class="index">{{Cart::instance('cart')->subtotal()}}</b>
                @if(Session::has('coupon'))
                    <p class="summary-info"><span class="title">Discount({{Session::get('coupon')['code']}})</span><b class="index"> -{{$discount,2}}</b></p>
            ...
            @if(!Session::has('coupon'))
                    <div class="checkout-info">
                        <label class="checkbox-field">
                            <input class="frm-input " name="have-code" value="1" id="have-code" value="" type="checkbox" wire:model="haveCouponCode"><span>I have coupon code</span>
                        </label>
                        @if($haveCouponCode==1)
                        <div class="summary-item">
                            <form wire:submit.prevent="applyCouponCode">
                                <h4 class="title-box">Coupon box</h4>
                                @if(Session::has('coupon_message'))
                                    <div class="alert alert-danger" role="danger">{{Session::get('coupon_message')}}</div>
                                @endif
                                <p class=" wire:model=haveCouponCode">
                    ...
                    <ul class="products-cart">
                        @foreach(Cart::instance('saveForLater')->content() as $item)
                            <li class="pr-cart-item">
                                ...
                                <div class="delete">
                                    <a href="#" wire:click.prevent="deleteToCart('{{$item->rowId}}')" class="btn btn-delete" title="">
